"""doc:
    ...
    2024.4.26
    Add the Parameter "mode" to the function KEB_SpatialMode
    mode=0:Matrix without considering the boundary condition.This mode is setting for caculating the Normalization factor Q in receptivity
    mode=1:Matrix consider the boundary condition 
    """
module KEB_TimeMode
...
module KEB_SpatialMode
    ...
    using SparseArrays
    export KEB_LST_ALL
    function KEB_LST_ALL(baseflow,N,ω,β,R,Ro,Co,mode)
        θ = range(0,length=N+1,stop=pi)
        x = reshape(-cos.(θ), N+1, 1)
        c = [2; ones(N-1, 1) ; 2] .* (-1) .^ (0:N)
        X = repeat(x, 1, N+1);
        ...
        A3=sparse(A3);
        A4=sparse(A4);
        return A0,A1,A2,A3,A4,dU,dV,dW
     end
...
module roughness_receptivity
    using LinearAlgebra
    using BSplineKit
    using SparseArrays
    export KEB_LST_Rec
    function KEB_LST_Rec(baseflow,N,ω,β,R,Ro,Co)
    θ = range(0,length=N+1,stop=pi)
    x = reshape(-cos.(θ), N+1, 1)
    c = [2; ones(N-1, 1) ; 2] .* (-1) .^ (0:N)
    X = repeat(x, 1, N+1);
    ...
    A3=[L3_1 L3_2 ;L3_3 L3_4 ;]
    A4=[L4_1 L4_2 ;L4_3 L4_4 ;]
    A0=A0[setdiff(1:end , (2,N,N+1,N+2,2N+2)),setdiff(1:end , (1,2,N,N+1,N+2,2N+2))]
    A1=A1[setdiff(1:end , (2,N,N+1,N+2,2N+2)),setdiff(1:end , (1,2,N,N+1,N+2,2N+2))]
    A2=A2[setdiff(1:end , (2,N,N+1,N+2,2N+2)),setdiff(1:end , (1,2,N,N+1,N+2,2N+2))]
    A3=A3[setdiff(1:end , (2,N,N+1,N+2,2N+2)),setdiff(1:end , (1,2,N,N+1,N+2,2N+2))]
    A4=A4[setdiff(1:end , (2,N,N+1,N+2,2N+2)),setdiff(1:end , (1,2,N,N+1,N+2,2N+2))]
    A0=sparse(A0);
    A1=sparse(A1);
    A2=sparse(A2);
    A3=sparse(A3);
