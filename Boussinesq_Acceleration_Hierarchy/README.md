...
This workspace supports a new study of error-controlled Boussinesq model hierarchies in rotating frames. The intended research chain is

```text
traditional-model saddle-node
    -> acceleration decomposition and scaling
    -> nested density-acceleration closures
    -> base-flow bifurcation sensitivity
    -> modal sensitivity
    -> error indicator and model-selection rule
```
...
1. define the dimensional rotating-frame acceleration decomposition and the exact Rossby-number convention;
2. introduce continuous coefficients multiplying each density--acceleration contribution;
3. reproduce the traditional and acceleration-consistent steady base flows as limiting coefficient choices;
4. compute the first fold location as those coefficients and `Ro` vary;
5. evaluate whether fold shifts collapse against candidate density--acceleration numbers.
