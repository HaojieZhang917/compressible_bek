...
TWODISK = PROJECT_ROOT / "rotor_stator"
TWODISK_DATA = TWODISK / "data"
VONKARMAN = PROJECT_ROOT / "von_karman"
OUT = PROJECT_ROOT / "cross_model" / "data" / "dynamical_singularity_comparison"
sys.path.insert(0, str(VONKARMAN / "scripts"))
sys.path.insert(0, str(TWODISK / "scripts"))
...
def first_fold_by_domain() -> dict[float, tuple[float, float]]:
    rows = read_csv_dicts(
        VONKARMAN / "data" / "finite_domain_branches" / "turning_points_by_zmax.csv"
    )
    return {
...
def scan_rotor_stator_near_folds(collocation_order: int = 101) -> list[dict]:
    ...
    folds = json.loads(
        (TWODISK_DATA / "boussinesq_singularity_results" /
    ...
    solver = TwoDiskBoussinesqSolver(
        ...
        TWODISK / "reference" / "baseflow_Res1000.npz",
    )
